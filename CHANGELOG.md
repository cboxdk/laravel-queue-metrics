...
- `cleanupStaleWorkers()` no longer loads the entire stale-worker set into memory. On a churning fleet — workers SIGKILLed on scale-in never unregister, and the `workers:all` index only shrinks through this command — the unbounded `getSortedSetByScore()` load grew until it exceeded the process memory limit and OOM-killed the scheduled `queue-metrics:cleanup-stale-workers` before it deleted anything, so the index never shrank and every run died the same way. The Redis driver now trims the index server-side (`ZREMRANGEBYSCORE`), which is all it needs because `getActiveWorkers()` reads that index and the per-worker hashes expire on their TTL. The database driver deletes in bounded chunks — the worker hash, index member and active-worker rows together — because its `getActiveWorkers()` reads the active set plus the hash (whose TTL is far longer than the stale threshold), so those must be removed promptly rather than left to expire.
...
- Redis Cluster support for the metrics store (phpredis): `scanKeys()` scans every master node, queue-depth reads use the `{hash tag}` key that Laravel's `RedisQueue` writes on a cluster, and the transaction/pipeline write paths issue commands individually so they work on a slot-routed connection. The single-node connection path is unchanged, and all supported Laravel versions are retained — driving queues on a cluster requires Laravel 13 (earlier `RedisQueue` versions cannot push to a cluster), while cluster-hosted metrics storage works everywhere. Known limitation: worker-heartbeat writes still use a multi-key Lua script that a multi-slot cluster rejects; heartbeats require a single-node (or single-slot) Redis for now.
- Cluster + single-node Redis test coverage, including a CI job that runs the Redis suite against a real 3-master cluster on Laravel 13.
